...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:res="http://www.esri.com/metadata/res/" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:srv="http://www.isotc211.org/2005/srv" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gts="http://www.isotc211.org/2005/gts" xmlns:gml="http://www.opengis.net/gml" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2005/xpath-functions">
  ...
  <ul>
    ...
    <!-- DIS version of the DTD doesn't account for data and service subclasses of MD_Identification. 
		...
		<xsl:call-template name="TOC-HEADING">
			...
			<xsl:with-param name="label"><xsl:value-of select="fn:string('idResIdInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('idResource')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Spatial Representation Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$spatRepInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('idSpatRepInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('idRepresentation')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Content Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$contInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDContentInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDDescription3')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Reference System Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$refSysInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDRefSysInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDRefSys')"/></xsl:with-param>
		</xsl:call-template>
		
    <!-- Data Quality Information -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$dqInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDDataQualInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDDataQuality')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Distribution Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$distInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDDistInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDDist')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Portrayal Catalogue Reference  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$porCatInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDPorCatRef')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDCatalogue')"/></xsl:with-param>
		</xsl:call-template>

    <!-- Application Schema Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$appSchInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDAppSchInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDSchema')"/></xsl:with-param>
		</xsl:call-template>
		
    <!-- Metadata Extension Information  -->
		<xsl:call-template name="TOC-HEADING">
			<xsl:with-param name="nodes" select="$mdExtInfo"/>
			<xsl:with-param name="label"><xsl:value-of select="fn:string('IDMetaExtInfo')"/></xsl:with-param>
			<xsl:with-param name="sub-label"><xsl:value-of select="fn:string('IDExtension')"/></xsl:with-param>
		</xsl:call-template>
