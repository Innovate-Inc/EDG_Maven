...
public class LuceneIndexAdapter extends CatalogIndexAdapter {
  ...
  public void publishDocument(String uuid, 
    ...
    LogUtil.getLogger().info("Publishing document with uuid: "+uuid);
    if (uuid.length() == 0) {
    ...
    try {
      ...
      if (bReadDB) {
        ...
        this.getRequestContext().getCatalogConfiguration().getResourceTableName();
                String sql = "SELECT SITEUUID, TITLE, OWNER FROM " + cfg.getResourceTableName() + " WHERE DOCUUID=?";

                sql = "SELECT res.docuuid,res.SITEUUID, res.TITLE, res.OWNER,children.list as childlist,parents.list as parentlist,collection.list as collection FROM gpt_resource res "
                        +" LEFT JOIN (SELECT docuuid, array_to_string(array_agg(child_docuuid), ' ') as list FROM inno_collection_member cm LEFT JOIN inno_collections inc ON (cm.col_id=inc.col_id) WHERE inc.approved='Y' GROUP BY docuuid) children ON (res.docuuid=children.docuuid) "
                        +" LEFT JOIN (SELECT child_docuuid, array_to_string(array_agg(docuuid), ' ') as list FROM inno_collection_member cm LEFT JOIN inno_collections inc ON (cm.col_id=inc.col_id) WHERE inc.approved='Y' GROUP BY child_docuuid) parents ON (res.docuuid=parents.child_docuuid) "
                        +" LEFT JOIN ("
                        +" SELECT tmp.docuuid,array_to_string(array_agg(tmp.list), ' ') list FROM ("
                        +" SELECT cm.docuuid,array_to_string(array_agg(DISTINCT cm.col_id), ' ') as list  FROM inno_collection_member  cm LEFT JOIN inno_collections inc ON (cm.col_id=inc.col_id) WHERE inc.approved='Y'  GROUP BY docuuid "
                        +" UNION SELECT cm.child_docuuid,array_to_string(array_agg(DISTINCT cm.col_id), ' ') as list  FROM inno_collection_member  cm LEFT JOIN inno_collections inc ON (cm.col_id=inc.col_id) WHERE inc.approved='Y'  GROUP BY child_docuuid) tmp"
                        +" GROUP BY tmp.docuuid"
        ...
        if (!sParentIdentifier.isEmpty()) {
          ...
          document.add(fld);
                ...
                if (rs.next()) {
                    ...
                    dbVal = Val.chkStr(rs.getString("OWNER"));
                    if (dbVal.length() > 0) {
                        //storeables.ensure(meanings,Storeables.FIELD_SITEUUID).setValue(dbVal);
                        fldName = "sys.owner";
                        LOGGER.log(Level.FINER, "Appending field: {0} ={1}", new Object[]{fldName, dbVal});
                    ...
                    dbVal = Val.chkStr(rs.getString("CHILDLIST"));
                    if (dbVal.length() > 0) {
                        //LOGGER.log(Level.FINER, "Deewendra childlist : "+dbVal);
                        storeables.ensure(meanings, "sys.childlist").setValues(dbVal.split(" "));
                        
                        //storeables.ensure(meanings,Storeables.FIELD_SITEUUID).setValue(dbVal);
                        //fldName = "sys.childlist";
                        //LOGGER.log(Level.FINER, "Appending field: {0} ={1}", new Object[]{fldName, dbVal});
                        //fld = new Field(fldName, dbVal, Field.Store.YES, Field.Index.NOT_ANALYZED, Field.TermVector.YES);
                        //document.add(fld);
                    }

                    dbVal = Val.chkStr(rs.getString("PARENTLIST"));
                    if (dbVal.length() > 0) {
                        storeables.ensure(meanings, "sys.parentlist").setValues(dbVal.split(" "));
                    }
                    
                    //Changes made by NETTY
                    dbVal = Val.chkStr(rs.getString("COLLECTION"));
                    if (dbVal.length() > 0) {
                        storeables.ensure(meanings, "sys.collection").setValues(dbVal.split(" "));
                    }

                    /*dbVal = Val.chkStr(rs.getString("USERNAME"));
                    if (dbVal.length() > 0) {
                    //storeables.ensure(meanings,Storeables.FIELD_SITEUUID).setValue(dbVal);
                    fldName = "sys.ownerName";
                    LOGGER.log(Level.FINER, "Appending field: {0} ={1}", new Object[]{fldName, dbVal});
                    fld = new Field(fldName,dbVal,Field.Store.YES,Field.Index.NOT_ANALYZED,Field.TermVector.NO);
                    document.add(fld);
                    }*/
        ...
        if (useCollections && hasCollections) {
          ...
          if (alCol.size() > 0) {
            ...
            for (IStoreable ist : storeables.collection()) {
                ...
                storeable.appendForWrite(document);//just adds field
            }
            //LOGGER.log(Level.FINER, "Deewendra END OF LOOP");
      // schema key
