  ...
  protected void executeQuery(DiscoveryQuery discoveryQuery) 
    ...
    try {
      ...
      if (!bAdmin && !acl.isPolicyUnrestricted()) {
          MetadataAccessPolicy map =  reqContext.getApplicationConfiguration().getMetadataAccessPolicy();
          String[] aclValues;
          if (map.getAccessPolicyType().equals(map.TYPE_PUBLIC_PROTECTED)) {
              boolean bNullServletRequestUser = false;
              if (reqContext.getServletRequest() == null) {
                  bNullServletRequestUser = true;
              }
              else {
                  if (((HttpServletRequest)(reqContext.getServletRequest())).getUserPrincipal() == null)  {
                      bNullServletRequestUser = true;
                  }
              }
              //if((reqContext.getUser() == null || reqContext.getUser().getName().trim().equals("")) && ((HttpServletRequest)(reqContext.getServletRequest())).getUserPrincipal() == null) {
              if((reqContext.getUser() == null || reqContext.getUser().getName().trim().equals("")) && bNullServletRequestUser) {
                  // not authenticated
                  aclValues = new String[] {};
              } else {
                  // authenticated
                  aclValues = new String[] {"g." + map.getAccessToGroupDN()};
              }
          } else {
              aclValues = acl.makeUserAcl();
          }
          
          filter = new AclFilter(Storeables.FIELD_ACL,aclValues);
          
          LOGGER.finer("aclValues");
          if (aclValues.length>0)
              LOGGER.finer(aclValues[0]);
          else
              LOGGER.finer("No aclValues");
      }

      LOGGER.finer("md acl");
      if(filter!=null) LOGGER.finer(filter.toString());
