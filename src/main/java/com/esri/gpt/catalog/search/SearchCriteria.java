...
/**
 * Gets the tags used in search page
 */
public ArrayList<ArrayList<String>> getTagscloud() {
    ...
    TreeMap<Integer,ArrayList<String>> sortedFrequencyList = new TreeMap<Integer,ArrayList<String>>();
    ...
    try{
        ...
        {
            String termName = arr.getJSONObject(i).getString("name");
            int termFrequency = arr.getJSONObject(i).getInt("documents");

            
            //added new
            if (!sortedFrequencyList.containsKey(termFrequency)) {
                ArrayList<String> currentList = new ArrayList<String>();
                currentList.add(termName);
                sortedFrequencyList.put(termFrequency, currentList);
                
            }
            else
            {
                ArrayList<String> currentList = sortedFrequencyList.get(termFrequency);
                currentList.add(termName);
        ...
        int count = 0;
        NavigableMap reversedFrequency = sortedFrequencyList.descendingMap();
        Iterator entries = reversedFrequency.entrySet().iterator();
        boolean bInsertAtFirst = true;
        int frequencyLevel = 0;
        while (entries.hasNext()) {
            if (count >= nTagCount) break;
            Entry thisEntry = (Entry) entries.next();
            ArrayList<String> currentList = sortedFrequencyList.get(thisEntry.getKey());
            for(int i=0; i<currentList.size();i++) {
                 ArrayList<String> oneTag = new ArrayList<String>();
                 
                 oneTag.add(currentList.get(i));
                 oneTag.add(thisEntry.getKey().toString());
                 oneTag.add(Integer.toString(frequencyLevel));
                 if (bInsertAtFirst) {
                    tagList.add(0,oneTag); 
                    bInsertAtFirst = false;
                 }
                 else {
                     tagList.add(tagList.size(),oneTag);
                     bInsertAtFirst = true;
                 }
                 count++;
                 if (count >= nTagCount) break;
            }
            frequencyLevel = frequencyLevel + 1;
        }
    }
    catch(Exception e){
